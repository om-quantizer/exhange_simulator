...
class ClientHandler:
    ...
    def _handle_client(self, conn, client_id):
        # Use makefile() to get a file-like object that buffers input by line.
        with conn:
            welcome_msg = (
            ...
            # Get a file-like object from the connection to ensure line buffering.
            ...
            try:
                for line in file_obj:
                    ...
                    if cmd in ("EXIT", "QUIT"):
                        ...
                        if success:
                            ack_msg = f"X: CANCEL ACK: Order {order_id} cancelled successfully.\n"
                        ...
                        try:
                            ...
                            continue
                        self.log.info(f"Processing EDIT command for order {order_id} from Client{client_id} to {new_price:.2f} and {new_quantity}")
                        success = self.engine.order_book.edit_order(order_id, new_price=new_price, new_quantity=new_quantity)
                        if success:
                            ack_msg = f"M: EDIT ACK: Order {order_id} modified to price {new_price:.2f}, quantity {new_quantity}.\n"
                            conn.sendall(ack_msg.encode())
                            self.log.info(f"M: Client{client_id} edited order {order_id} to {new_price:.2f} @ {new_quantity}")
                        else:
